...
const List = () => {
  ...
  document.title = `Levels | ${process.env.REACT_APP_SITE_NAME}`;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/level/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/level/delete/${delId}`);
  ...
  const handleSwitch = async (e) => {
    let input = e.target;
    const response = await api.update(`${process.env.REACT_APP_API_URL}/level/update/${input.getAttribute('data-id')}`, {
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/level/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
        ...
        <Container fluid>
          ...
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Levels</h5>
                </CardHeader>
                <CardBody>

                  {/* <Link to={"/level/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Level</Button>
                  </Link> */}
