...
const Emails = () => {
  ...
  document.title ="Emails | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/static/get-email`);
  ...
  const columns = useMemo(
    () => [
      ...
      {
        Header: "Subject",
        accessor: "template_subject",
        disableFilters: true,
        filterable: false,
      },
      {
        Header: "Last Updated",
        accessor: "last_update_on",
        disableFilters: true,
        filterable: false,
      },
      {
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/manage-email/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
            </div>
          )
        },
        disableFilters: true,
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Emails" pageTitle="Emails" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Emails</h5>
                </CardHeader>
                <CardBody>
