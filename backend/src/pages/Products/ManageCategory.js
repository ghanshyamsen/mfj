...
const Manage = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/product/category/get/${Id}`);
      if(response.status === "success") {
        const dataArray = response.data;
        populateFromData(dataArray);
      }else{
        history('/product/category/list');
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/product/category/update/${Id}`,formData);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/product/category/create`,formData);
    }
