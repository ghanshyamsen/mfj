...
const List = () => {
  ...
  document.title = `Products | ${process.env.REACT_APP_SITE_NAME}`;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/product/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/product/delete/${delId}`);
  ...
  const handleSwitch = async (e) => {
    let input = e.target;
    const response = await api.update(`${process.env.REACT_APP_API_URL}/product/update/${input.getAttribute('data-id')}`, {
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/product/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Products" pageTitle="Product" />
              ...
              <Card>
                ...
                <CardBody>
                  ...
                  <Link to={"/product/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Product</Button>
                  </Link>
