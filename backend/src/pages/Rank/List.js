...
const List = () => {
  ...
  document.title = `Ranks | ${process.env.REACT_APP_SITE_NAME}`;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/rank/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/rank/delete/${delId}`);
  ...
  const handleSwitch = async (e) => {
    let input = e.target;
    const response = await api.update(`${process.env.REACT_APP_API_URL}/rank/update/${input.getAttribute('data-id')}`, {
         ...
         accessor: (cellProps) => {
          return (
            <div className="form-check form-switch">
              <Input className="form-check-input" type="checkbox" role="switch" data-id={cellProps.id} onChange={handleSwitch} id={"SwitchCheck"+cellProps.id} defaultChecked={(cellProps.default)?true:false}  />
              &nbsp; { (cellProps.default)?<span className="ms-2 badge badge-soft-success">Active</span>:<span className="badge badge-soft-danger">Disabled</span> }
            </div>
          )
        },
        disableFilters: true,
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/rank/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Ranks" pageTitle="Ranks" />
          ...
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Ranks</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/rank/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Rank</Button>
                  </Link>
