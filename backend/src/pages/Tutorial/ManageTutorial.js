...
const ManageTutorial = () => {
  ...
  document.title ="Manage Tutorial | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/tutorial/get/${Id}`);
      if(response.status === "success") {
        const dataArray = response.data;
        populateFromData(dataArray);
      }else{
        window.notify('error',response.message);
        history('/tutorial/master-list');
      }
    } catch (error) {
      console.error(error);
    }
  ...
  const fetchOption = async () => {
    try {
      const category = await api.get(`${process.env.REACT_APP_API_URL}/tutorial-category/get`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/tutorial/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/tutorial/create`,FormDataObj);
    }
  ...
  const validateForm = (data) => {
    ...
    if (!data.category || (optionData && !optionData.some(cat => cat.id === data.category))) {
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Manage Tutorial" pageTitle="" />
