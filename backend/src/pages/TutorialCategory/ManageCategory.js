...
const ManageCategory = () => {
  ...
  document.title ="Manage Tutorial Category | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/tutorial-category/get/${Id}`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/tutorial-category/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/tutorial-category/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/tutorial-category/master-list');
    }else{
  ...
  const validateForm = (data) => {
    ...
    if (!data.title.trim()) {
      errors.title = 'Tutorial Category name is required';
    }
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Manage Category" pageTitle="" />
