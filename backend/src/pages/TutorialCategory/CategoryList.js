...
const Category = () => {
  ...
  document.title ="Tutorial Category | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/tutorial-category/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/tutorial-category/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/tutorial-category/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Tutorial Category" pageTitle="Tutorial Category" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Tutorial Category</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/tutorial-category/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Tutorial Category</Button>
                  </Link>
