...
const Category = () => {
  ...
  document.title ="Activitie Category | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/activitie-category/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/activitie-category/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/activitie-category/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Activity Categories" pageTitle="Activity Categories" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Activity Categories</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/activitie-category/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Activity Category</Button>
                  </Link>
