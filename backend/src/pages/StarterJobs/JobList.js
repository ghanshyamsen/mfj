...
const List = () => {
  ...
  document.title ="Starter Job | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/starter-job/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/starter-job/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/starter-jobs/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Starter Jobs" pageTitle="Starter Jobs" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Starter Jobs</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/starter-jobs/manage"}>
