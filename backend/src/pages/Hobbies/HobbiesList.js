...
const Hobbies = () => {
  document.title = "Hobbies & Interest | " + process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(
        `${process.env.REACT_APP_API_URL}/hobbies/get`
      );
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(
      `${process.env.REACT_APP_API_URL}/hobbies/delete/${delId}`
    );
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link
                to={"/hobbies/manage/" + cellProps.id}
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb
            title="Hobbie & Interest List"
            pageTitle="Hobbies & Interest"
          />
          <Row>
            <Col xs={12}>
              {errorMsg ? (
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Hobbies & Interest</h5>
                </CardHeader>
                <CardBody>
                  <Link to={"/hobbies/manage"}>
                    <Button className="btn-border" outline>
                      <i className="ri-add-line"></i> Add Hobbie & Interest
                    </Button>
                  </Link>
