...
const ManageHobbies = () => {
  ...
  document.title ="Manage Hobbies & Interest | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/hobbies/get/${Id}`);
  ...
  const fetchOption = async () => {
    try {
      const category = await api.get(`${process.env.REACT_APP_API_URL}/hobbies-category/get`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/hobbies/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/hobbies/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/hobbies/master-list');
    }else{
  ...
  const validateForm = (data) => {
    ...
    if (!data.title.trim()) {
      errors.title = 'Hobbie & Interest name is required';
    }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Manage Hobbies & Interest" pageTitle="" />
          <Row>
            <Col xs={12}>
              <Card>
                <CardBody>
                  ...
                  <div className="p-2 mt-4">
                    <Form onSubmit={handleSubmit} encType="multipart/form-data">
                      ...
                      <div className="form-group mb-3" key="image">
                        ...
                        {
                          // eslint-disable-next-line
                          (typeof formData.image === 'string' && formData.image && formData.image!=="" && formData.image!=="null")?<div style={{width:"200px"}} className="p-2 m-1 rounded-3 bg-dark"><img src={`${process.env.REACT_APP_MEDIA_URL}hobbies/${formData.image}`} className="w-100" /></div>:null
                        }
                      </div>
