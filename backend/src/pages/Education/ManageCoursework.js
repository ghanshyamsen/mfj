...
const ManageCoursework = () => {

  document.title ="Manage Courseworks | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/coursework/get/${Id}`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/coursework/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/coursework/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/education/coursework-list');
    }else{
  ...
  const validateForm = (data) => {
    ...
    if (!data.title.trim()) {
      errors.title = 'Coursework name is required';
    }
  ...
  document.title = `Manage Coursework | ${process.env.REACT_APP_SITE_NAME}`;
                  ...
                  <div className="p-2 mt-4">
                    <Form onSubmit={handleSubmit} encType="multipart/form-data">
                      ...
                      <div className="form-group mb-3" key="title">
                        ...
                        <Input type="text" id="title" name="title" maxLength="50"  onChange={handleChange} value={formData.title || ''}/>
...
export default ManageCoursework;