...
const Academic = () => {
  ...
  document.title ="Academics | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/academic/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/academic/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/education/manage-academic/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Academic List" pageTitle="Academics" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Academics</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/education/manage-academic"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Academic</Button>
                  </Link>
