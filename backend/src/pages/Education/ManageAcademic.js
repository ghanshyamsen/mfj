...
const ManageAcademic = () => {
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/academic/get/${Id}`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/academic/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/academic/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/education/academic-list');
    }else{
  ...
  const validateForm = (data) => {
    ...
    if (!data.title.trim()) {
      errors.title = 'Academic name is required';
    }
  ...
  document.title = `Manage Academic | ${process.env.REACT_APP_SITE_NAME}`;
