...
const List = () => {
  ...
  document.title = "Learning Path | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/lms/path/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/lms/path/delete/${delId}`);
  ...
  const handleSwitch = async (e) => {
    let input = e.target;
    const response = await api.update(`${process.env.REACT_APP_API_URL}/lms/path/update/${input.getAttribute('data-id')}`, {
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/lms/learning-path/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Learning Path" pageTitle="Learning Path" />
          ...
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Learning Path</h5>
                </CardHeader>
                <CardBody>

                  {/* <Link to={"/lms/learning-path/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Learning Path</Button>
                  </Link> */}
