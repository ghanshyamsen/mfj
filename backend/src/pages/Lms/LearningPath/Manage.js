...
const Manage = () => {
  ...
  document.title = `Manage Learning Path | ${process.env.REACT_APP_SITE_NAME}`;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/lms/path/get/${Id}`);
      if(response.status) {
        ...
        history('/lms/learning-path/list');
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/lms/path/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/lms/path/create`,FormDataObj);
    }
  ...
  const validateForm = (data) => {
    ...
    if (!data.credit_price|| data?.credit_price === 'e') {
      errors.credit_price = 'Credit is required';
    }else{
    ...
    /* if (!data.expiration_period || data?.expiration_period === 'e') {
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Manage Learning Path" pageTitle="" />
          <Row>
            <Col xs={12}>
              <Card>
                <CardBody>
                  <div className="p-2 mt-4">
                    <Form onSubmit={handleSubmit} encType="multipart/form-data">
                      ...
                      <Row>
                            ...
                            {errors.thumbnail && <div className="error" style={{ color: "red" }}>{errors.thumbnail}</div>}
                            {
                              ...
                              (typeof formData.thumbnail === 'string' && formData.thumbnail && formData.thumbnail!=="" && formData.thumbnail!=="null")?
                                <div style={{width:"200px"}} className="p-2 m-1 rounded-3 bg-dark">
                                  <img src={`${process.env.REACT_APP_MEDIA_URL}lms/path/${formData.thumbnail}`} className="w-100" />
                                </div>:null
                            }
                          </div>
                        </Col>
                        ...
                        <Col xs={12} sm={4}>
                          <div className="form-group mb-3" key="logo">
                            ...
                            {errors.logo && <div className="error" style={{ color: "red" }}>{errors.logo}</div>}
                            {
                              // eslint-disable-next-line
                              (typeof formData.logo === 'string' && formData.logo && formData.logo!=="" && formData.logo!=="null")?
                                <div style={{width:"200px"}} className="p-2 m-1 rounded-3 bg-dark">
                                  <img src={`${process.env.REACT_APP_MEDIA_URL}lms/path/${formData.logo}`} className="w-100" />
                                </div>:null
                            }
                          </div>
                        </Col>
                        ...
                        <Col xs={12} sm={4}>
                          <div className="form-group mb-3" key="badge">
                            <Label htmlFor="badge" className="form-label">Badge</Label>
                            <Input type="file" id="badge" name="badge"  onChange={handleChange}/>
                            {errors.badge && <div className="error" style={{ color: "red" }}>{errors.badge}</div>}
                            {
                              // eslint-disable-next-line
                              (typeof formData.badge === 'string' && formData.badge && formData.badge!=="" && formData.badge!=="null")?
                                <div style={{width:"200px"}} className="p-2 m-1 rounded-3 bg-dark">
                                  <img src={`${process.env.REACT_APP_MEDIA_URL}lms/path/${formData.badge}`} className="w-100" />
                                </div>:null
                            }
                          </div>
                        </Col>
                      ...
                      <div className="form-group mb-3" key="skills">
                      ...
                      <div className="form-group mb-3" key="short_description">
                        <Label htmlFor="short_description" className="form-label">Short Description</Label>
                        <Input type="text" id="short_description" name="short_description" maxLength="100" onChange={handleChange} value={formData.short_description || ''}/>
                        {errors.short_description && <div className="error" style={{ color: "red" }}>{errors.short_description}</div>}
                      </div>

                      <div className="form-group mb-3" key="long_description">
                        <Label htmlFor="expiration_period" className="form-label">Long Description</Label>
                        <JoditEditor
                          ref={editor}
                          ...
                          onBlur={(newContent) =>  setFormData({ ...formData, ['long_description']: newContent }) } // preferred to use only this option to update the content for performance reasons
                          onChange={(newContent) => { setFormData({ ...formData, ['long_description']: newContent }) }}
                        />
                        {errors.long_description && <div className="error" style={{ color: "red" }}>{errors.long_description}</div>}
                      </div>
