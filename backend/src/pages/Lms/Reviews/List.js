...
const List = () => {
  ...
  document.title = "Reviews | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/lms/review/get`);
  ...
  const handleSwitch = async (e) => {
    let input = e.target;
    const response = await api.update(`${process.env.REACT_APP_API_URL}/lms/review/update/${input.getAttribute('data-id')}`, {
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              ...
              <Link to={"/lms/learning-material/manage?skill="+cellProps.id} className="link-success fs-15">
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Reviews" pageTitle="Reviews" />
