...
const ManageCategory = () => {
  ...
  document.title ="Manage Hobbies Category | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/hobbies-category/get/${Id}`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/hobbies-category/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/hobbies-category/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/hobbies-category/master-list');
    }else{
  ...
  const validateForm = (data) => {
    ...
    if (!data.title.trim()) {
      errors.title = 'Hobbies category name is required';
    }
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
        <BreadCrumb title="Manage Hobbies Category" pageTitle="" />
          <Row>
            <Col xs={12}>
              <Card>
                <CardBody>
