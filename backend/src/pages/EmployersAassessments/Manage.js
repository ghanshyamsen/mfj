...
const Manage = () => {
  ...
  document.title ="Manage Employers Assessments | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/employers-assessments/get/${Id}`);
      if(response.status) {
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/employers-assessments/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/employers-assessments/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/employers-assessments');
    }else{
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
        <BreadCrumb title="Manage Employers Assessments" pageTitle="" />
          <Row>
            <Col xs={12}>
              <Card>
                <CardBody>
