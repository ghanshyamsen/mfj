...
const List = () => {
  ...
  document.title ="Employers Assessments | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/employers-assessments/get`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/employers-assessments/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
            </div>
          )
        },
        disableFilters: true,
  ...
  return (
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Employers Assessments List" pageTitle="Employers Assessments" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Employers Assessments</h5>
                </CardHeader>
                <CardBody>

                  {/* <Link to={"/employers-assessments/manage"}>
