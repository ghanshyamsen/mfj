...
const List = () => {
  ...
  document.title = "Article Categories | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/article/category/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/article/category/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/article-category/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
    ...
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Article Categories" pageTitle="Article Category" />
          ...
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Article Categories</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/article-category/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Article Category</Button>
                  </Link>
