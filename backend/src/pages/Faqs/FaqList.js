...
const Faq = () => {
  ...
  document.title ="Faq's | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/faq/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/faq/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/faq/manage/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Faq's" pageTitle="Faq's" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Faq's</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/faq/manage"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Faq</Button>
                  </Link>
                  ...
                  <TableContainer
                    ...
                    tableClassName="table table-hover table-centered align-middle table-nowrap mb-0"
