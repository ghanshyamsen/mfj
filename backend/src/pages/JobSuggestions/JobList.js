...
const JobList = () => {
  document.title = "Job Suggestions | " + process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(
        `${process.env.REACT_APP_API_URL}/job/suggestion/get`
      );
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(
      `${process.env.REACT_APP_API_URL}/job/suggestion/delete/${delId}`
    );
  ...
  const handleSwitch = async (e) => {
    ...
    const response = await api.update(
      ...
      }/job/suggestion/update/${input.getAttribute("data-id")}`,
      {
    ...
    if (response.status) {
      if (input.checked) {
        ...
        $(input)
          .parent("div")
          .append(`<span class="ms-2 badge badge-soft-success">Active</span>`);
      } else {
        //code
        $(input).next("span").remove();
        $(input)
          .parent("div")
          .append(`<span class="ms-2 badge badge-soft-danger">Disabled</span>`);
      }
    }
  };
          ...
          return (
            <div className="form-check form-switch">
              <Input
                ...
                defaultChecked={cellProps.job_status ? true : false}
              />
              &nbsp;{" "}
              {cellProps.job_status ? (
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link
                to={"/job/manage-suggestion/" + cellProps.id}
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Job Suggestions" pageTitle="Jobs" />
          ...
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Job Suggestions</h5>
                </CardHeader>
                <CardBody>
                  <Link to={"/job/manage-suggestion"}>
                    <Button className="btn-border" outline>
                      <i className="ri-add-line"></i> Add Job Suggestion
                    </Button>
                  </Link>
