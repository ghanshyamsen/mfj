...
const Manage = () => {
  ...
  document.title ="Manage Personality Profile | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/personality-profile/get/${Id}`);
      if(response.status) {
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/personality-profile/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/personality-profile/create`,FormDataObj);
    }
  ...
  const validateForm = (data) => {
    ...
    for(let i = 0; i < 3; i++){
      if (!data?.personality_type?.[i] || (data?.personality_type?.[i] && !data?.personality_type?.[i].trim())) {

        if(!errors.personality_type){
          errors.personality_type = [];
        }

        errors.personality_type[i] = `Personality Type (${alfa[i]}) is required`;
      }
    }
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Manage Personality Profile" pageTitle="" />
                        ...
                        <Col sm={6}>
                          {[0, 1, 2, 3].map((index) => (
                            <div className="form-group mb-3" key={index}>
                              <Label htmlFor={`personality_type${index}`} className="form-label">
                                {`Interpretation ${alfa[index]}`}
                              </Label>
                              <Input type="select" id={`personality_type${index}`} name={`personality_type[${index}]`} onChange={handleChange} value={formData?.personality_type?.[index] || ''}>
                                <option value="">Select</option>
                                {options && options.map((element, index) => (
                                  <option key={index} value={element.id}>{element.title}</option>
                                ))}
                              </Input>
                              {errors?.personality_type?.[`${index}`] && (
                                <div className="error" style={{ color: 'red' }}>
                                  {errors.personality_type[`${index}`]}
                                </div>
                              )}
