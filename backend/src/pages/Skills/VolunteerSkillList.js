...
const Skills = () => {
  ...
  document.title ="Volunteer Skills | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/skills/volunteer/get`);
  ...
  const handleConfirm = async () => {
    ...
    const response = await api.delete(`${process.env.REACT_APP_API_URL}/skills/volunteer/delete/${delId}`);
        ...
        accessor: (cellProps) => {
          return (
            <div className="hstack gap-3 flex-wrap">
              <Link to={"/skills/manage-volunteer-skill/"+cellProps.id} className="link-success fs-15"><i className="ri-edit-line"></i></Link>
  ...
  return (
    <React.Fragment>
      ...
      <div className="page-content">
        <Container fluid>
          <BreadCrumb title="Volunteer Skills List" pageTitle="Volunteer Skills" />
          <Row>
            <Col xs={12}>
              ...
              <Card>
                <CardHeader>
                  <h5 className="card-title mb-0">Volunteer Skills</h5>
                </CardHeader>
                <CardBody>

                  <Link to={"/skills/manage-volunteer-skill"}>
                    <Button className="btn-border" outline><i className="ri-add-line"></i> Add Volunteer Skill</Button>
                  </Link>
