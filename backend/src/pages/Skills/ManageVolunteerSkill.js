...
const ManageSkill = () => {
  ...
  document.title ="Manage Volunteer Skills | "+process.env.REACT_APP_SITE_NAME;
  ...
  const fetchData = async () => {
    try {
      const response = await api.get(`${process.env.REACT_APP_API_URL}/skills/volunteer/get/${Id}`);
  ...
  const handleSubmit =  async (event) => {
    ...
    let response;
    if(Id){
      response =  await api.update(`${process.env.REACT_APP_API_URL}/skills/volunteer/update/${Id}`,FormDataObj);
    }else{
      response =  await api.create(`${process.env.REACT_APP_API_URL}/skills/volunteer/create`,FormDataObj);
    }

    // eslint-disable-next-line
    if(response.status){
      window.notify('success',response.message);
      history('/skills/volunteer-list');
    }else{
  ...
  const validateForm = (data) => {
    ...
    if (!data.title.trim()) {
      errors.title = 'Volunteer Skill name is required';
    }
    ...
    <React.Fragment>
      <div className="page-content">
        <Container fluid>
          ...
          <BreadCrumb title="Manage Volunteer Skills" pageTitle="" />
