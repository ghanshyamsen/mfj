...
const PAGE_HEIGHT = 820; // Height in pixels for A4 at 96 DPI
const BOTTOM_MARGIN = 25; // Additional margin to avoid content overflow
const HEADER_HEIGHT = 180; // Approximate header height in pixels
const OUTER_PADDING = 15;

function ClassicResume({resume, user}) {
        ...
        return (
            <div className='cr_top_row'>
                <div className='user_block'>
                    {user && user.profile_image_visible && <div className='user_img'> <img src={data.image} alt="" /> </div>}
                    <p className='mb-0 user_name'> {data.name} </p>
                </div>
                <div className='contact_info'>
                    <p className=''><span className="icon"><img src={Phone} alt="" /></span> <span>{data.phone_number}</span> </p>
                    <p className=''><span className="icon"><img src={Email} alt="" /></span> <span>{data.email}</span> </p>
                    <p className=''><span className="icon"><img src={mapMarker} alt="" /></span> <span>{data.location}</span> </p>
                </div>
            </div>
        )
    }
    ...
    const [dom, setDom] = useState(
        <div id='resumeContent' className={`${isBlackAndWhite ? 'blackAndWhite' : ''}`} ref={printRef}>
            <div className="new-page-wrapper a4 mb-3 classic_resume_content" style={{ pageBreakBefore: 'always',pageBreakAfter: 'always'}}>
                      ...
                      Array.from(el.children).map(el => {
                          ...
                          Array.from(el.children).map(el => {
                              ...
                              if($(el).hasClass('cr_top_row')){
                                  ...
                                  LEFT_CONTENT_HEIGHT += 20;
                                  RIGHT_CONTENT_HEIGHT += 20;
                                  const subDiv = createCustomElement(el);
                                  Array.from(el.children).map(el => {
                                      if($(el).hasClass('uic_default_right_content')){
    ...
    const splitScreen = () => {
      if (dom) {
        const Html = printRef.current.children;
        const elements = $(Html).clone();
        const qrHtml = elements.find('.qr-block').html();

        const W = createCustomElement('div');

        let LEFT_CONTENT_HEIGHT = 0;
        let RIGHT_CONTENT_HEIGHT = 0;
        let pageCreate = false;
        let i = 0;
                                    ...
                                    if (getAbsoluteHeight(el) + LEFT_CONTENT_HEIGHT >= PAGE_HEIGHT) {
                                      if (el.children.length > 0) {
                                        ...
                                        Array.from(el.children).map(child => {
                                          if (getAbsoluteHeight(child) + LEFT_CONTENT_HEIGHT >= PAGE_HEIGHT) {
                                            pageCreate = true;
                                            return;
                                          } else {
                                            $(SMALLDIV).append(child.cloneNode(true));
                                            LEFT_CONTENT_HEIGHT += getAbsoluteHeight(child);
                                            child.remove();
                                          }
                                        });
                                        if (SMALLDIV.children.length > 0) {
                          ...
                          Array.from(el.children).map(el => {
                            if (!pageCreate) {
                              if (getAbsoluteHeight(el) + RIGHT_CONTENT_HEIGHT > PAGE_HEIGHT) {
                                ...
                                Array.from(el.children).map(child => {
                                  if (!pageCreate) {
                                    if (getAbsoluteHeight(child) + RIGHT_CONTENT_HEIGHT >= PAGE_HEIGHT) {
                                      pageCreate = true;
                                      return;
                                    } else {
                                      $(DIV).append(child.cloneNode(true));
                                      RIGHT_CONTENT_HEIGHT += getAbsoluteHeight(child);
                                      child.remove();
                                    }
                                  }
                                });
