...
const FriendlyProfile = ({ resume, user }) => {
    ...
    const Header = ({ data }) => (
        <div className='user_info_content section'>
            <div className='uic_default_content section'>
                <AboutMe key="aboutme" data={resume.objective_summary} position="left" />
    ...
    const [dom, setDom] = useState(
        <div id='resumeContent' ref={printRef}>
            <div className="new-page-wrapper a4 mb-3 friendly_profile_resume_page" style={{ pageBreakBefore: 'always',pageBreakAfter: 'always'}}>
                                                                    ...
                                                                    if((getAbsoluteHeight(el) + LEFT_CONTENT_HEIGHT) >= PAGE_HEIGHT){
                                                                        ...
                                                                        if(el.children.length > 0 && $(el).hasClass('voln') && notTag(el)){
                                                                            ...
                                                                            Array.from(el.children).map(el => {
                                                                                ...
                                                                                if(el.children.length > 0 && $(el).hasClass('voln') && notTag(el)){
                                                                                    const SMALLDIVM = createCustomElement(el);
                                                                                    ...
                                                                                    Array.from(el.children).map(el => {
                                                                                        if((getAbsoluteHeight(el) + LEFT_CONTENT_HEIGHT) >= PAGE_HEIGHT){
                                                                                            ...
                                                                                            $(SMALLDIVM).append(el.cloneNode(true));
                                                                                            LEFT_CONTENT_HEIGHT += getAbsoluteHeight(el);
                                                                                            el.remove();
                                                                                        }
                                                                                    })

                                                                                    if(SMALLDIVM.children.length > 0){
                                                                                        $(SMALLDIV).append(SMALLDIVM.cloneNode(true));
                                                                                    }
...
export default FriendlyProfile;
