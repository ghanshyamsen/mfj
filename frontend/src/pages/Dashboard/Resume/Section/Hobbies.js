...
function Hobbies({data, maxHeight}) {
    ...
    useEffect(() => {
        const handleEditClick = (e) => {
          ...
          if (editBtn && editBtn.dataset.edit) {
            ...
            if(section === 'hobbies'){
                navegation('/hobbies-and-interests?edit=/resume/'+key);
            }
          }
        };
    ...
    return (
        ...
        <div className='linfo_block section clone' style={contentStyle}>
            <h2 className='heading push d-flex align-items-center justify-content-between'>
                <span> Hobbies </span>
                <a href="?edit=hobbies" className="edit-link edit_icon" data-edit="hobbies"><img src={Edit} alt="edit" /></a>
            </h2>
            {
                data.map((value, index) => (
                    <p key={index} className="push">
                        <span className='hobi_img'>{value.image  && <img src={value.image} alt={`hi${index}`} /> }</span>
