...
const HEADER_HEIGHT = 189; // Approximate header height in pixels
const OUTER_PADDING = 16;
const PAGE_HEIGHT = 820;
const ElegantCurve = ({ resume, user }) => {
    ...
    const Header = ({ data }) => (
        <div className='user_info_content section'>
            <div className='uic_default_content section'>
                ...
                <AboutMe key="aboutme" data={resume.objective_summary} position="left" className={isDownloading ? 'active-class' : ''}  showEditIcon={isDownloading}/>
                <WorkExperience key="workexperience" data={resume.work_experience} position="left" />
                <Education key="education" data={resume.education} position="left" />
                <VolunteerExperience key="volunteer" data={resume.volunteer_experience} position="left" />
                <Certificates key="certification" data={resume.certification} position="left" />
                <Awards key="awards_achievments" data={resume.awards_achievments} position="left" />
            ...
            <div className='uic_default_right_content section'>
                ...
                <Contact key="contact" data={resume.user_info} position="right" />
                <ExtracurricularActivities key="extracurricular" data={resume.extracurricular_activities} position="right" />
                <Skills key="skills" data={resume.skills} position="right" />
                <Hobbies key="hobbies" data={resume.hobbies} position="right" />
    ...
    const [dom, setDom] = useState(
        <div id='resumeContent' ref={printRef}>
            <div className="new-page-wrapper a4 mb-3 elegantcurve_resume_page" style={{ pageBreakBefore: 'always',pageBreakAfter: 'always'}}>
                                                        ...
                                                        if((getAbsoluteHeight(el)+RIGHT_CONTENT_HEIGHT) > PAGE_HEIGHT){
                                                            ...
                                                            if(!pageCreate){
                                                                Array.from(el.children).map(el => {
                                                                    if(!pageCreate){
                                                                        if((getAbsoluteHeight(el) + RIGHT_CONTENT_HEIGHT) >= PAGE_HEIGHT){
                                                                            if(el.children.length > 0 && notTag(el) && !$(el).hasClass('push') ){
