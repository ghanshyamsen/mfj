...
const TeenagerJobs = () => {
  ...
  const loadJobs = (keyword=false) => {
    ...
    fetch(`${process.env.REACT_APP_API_URL}/app/get-jobs?offset=${offset}&keyword=${searchTerm}&${params}`, requestOptions)
  ...
  // Set logos when jobs change or when you need to filter the jobs
  useEffect(() => {
    setIcons(() => {
      // Filter jobs that have a logo and return only the first 5 logos
      return jobs
      .filter(job => job.logo)  // Filter jobs with a logo
      .reduce((acc, job) => {
        // Check if the logo already exists in the accumulator
        if (!acc.some(item => item.logo === job.logo)) {
          acc.push(job); // Add job if its logo is not in the accumulator
        }
        return acc;
      }, [])
      .slice(0, 5)  // Take only the first 5 distinct logos
      .map(job => job.logo);  // Map to extract only the logos
    });
  }, [jobs]);
    ...
    <>
      <div className="jobs_page common_background_block">
        ...
        <div className="dashboard_content_block">
          ...
          <div className='job_info_block'>

            <FeaturedJobsSlider jobs={jobs} loadingSkeleton={loadingSkeleton} loading={loading} />


            {/* match jobs */}
            {loadingSkeleton ?
              <div className='matched_block'>
                <div className='match_title mb-3'> <Skeleton width="20%" height='32px' className='m-auto mb-2' /> </div>

                <div className='matched_img mb-3'>
                  <Skeleton shape="circle" width='48px' height='48px' />
                  <Skeleton shape="circle" width='48px' height='48px' />
                  <Skeleton shape="circle" width='48px' height='48px' />
                </div>

                <div className='match_text mb-3'> <Skeleton width="100%" height='20px' className='mb-2' /> </div>
                <Skeleton className='button' width="127px" height='36px' />
              </div>
            :
              <div className='matched_block'>
                  <h2 className='match_title'> Get matched </h2>
                  {icons.length > 0 && <div className='matched_img'>
                    {
                      icons.map((img, index) =>  <img key={'icon'+index} src={img} alt=""/>)
                    }
                  </div>}
                  <p className='match_text'> We've curated job opportunities tailored to your skills and career goals, enhancing your job search efficiency and increasing your chances of finding a fulfilling role. </p>
                  <Link to={`/matched-jobs`} style={{textDecoration:'none'}}>
                    <button className='btn get_btn'> Get matched </button>
                  </Link>
              </div>
            }
                ...
                <div className='application_ul_list'>
                    <ul>
                      <InfiniteScroll
                        ...
                        endMessage={
                          jobs.length > 0 && <p className='allseen' style={{ textAlign: 'center' }}>
